...
|---|---|---|
| Passphrase KDF | **Argon2id**, m=64 MiB, t=3, p=1, 16B salt, 32B output | Client-side. Per-user params are stored so they can be raised later, with a `needsUpgrade` re-wrap on next unlock — the same shape as `pinNeedsRehash` in `auth/pin.ts`. Browsers are single-threaded for this, so p=1. Above the OWASP 2025 floor and level with Bitwarden's defaults. Go: `golang.org/x/crypto/argon2`. Library choice for the browser: see the Argon2 measurement section below. |
