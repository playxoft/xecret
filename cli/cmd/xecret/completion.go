...
func fishCompletion() string {
	...
	// Two helpers instead of __fish_seen_subcommand_from, which matches a name
	// *anywhere* on the line and so cannot tell a verb from an argument that
	// happens to be spelled like one. `xecret secrets set list --<TAB>` offered
	// `secrets list`'s --json on the strength of the word "list" being a secret
	// name; accepting it failed with `flag provided but not defined: -json`.
	//
	...
	for _, command := range completionTree {
		for _, sub := range command.Subcommands {
			// Offered while slot 2 is still empty, and not once it names a verb.
			b.WriteString(fmt.Sprintf(
				"complete -c xecret -n '__xecret_at 1 %s; and __xecret_at 2 \"\"' -a %s -d '%s'\n",
				command.Name, sub.Name, sub.Description))
		}
	}
	b.WriteString("complete -c xecret -n '__xecret_at 1 import' -F\n")

	// One condition per command, so no command is offered a flag it does not
	// define. A parent's own flags apply only while no subcommand has been
	// named, which is exactly when its bare form is what would run.
	for _, command := range completionTree {
		condition := fmt.Sprintf("__xecret_at 1 %s", command.Name)
		if len(command.Subcommands) > 0 {
			condition += "; and __xecret_at 2 \"\""
		}
		for _, flagName := range command.Flags {
			b.WriteString(fmt.Sprintf("complete -c xecret -n '%s' %s\n", condition, fishFlag(flagName)))
		}
		for _, sub := range command.Subcommands {
			for _, flagName := range sub.Flags {
				b.WriteString(fmt.Sprintf("complete -c xecret -n '__xecret_at 1 %s; and __xecret_at 2 %s' %s\n",
